        ...
        <div class="offset-md-2 col-md-8">
            <div class="card">
                @if (isset($task))
                    ...
                    <div class="card-body">
                        <!-- Update Task Form -->
                        <form action="{{ url('taskUpdate') }}" method="POST">
                            @csrf
                        ...
                        <tbody>
                            @foreach ($tasks as $task)
                                <tr>
                                    <td>{{ $task->name }}</td>
                                    <td>
                                        <form action="/taskDelete/{{ $task->id }}" method="POST" class="d-inline">
                                            @csrf
                                            <button type="submit" class="btn btn-danger">
                                                <i class="fa fa-trash me-2"></i>Delete
                                            </button>
                                        </form>
                                        <form action="/taskEdit/{{ $task->id }}" method="POST" class="d-inline">
                                            @csrf
